...
- **Physics world separation** -- The physics world should maintain its own representation of objects (physics boresets) separate from game entities. A synchronization step maps between them.
...
class PhysicsWorld {
  ...
  step(dt) {
    ...
    for (const body of this.boresets) {
