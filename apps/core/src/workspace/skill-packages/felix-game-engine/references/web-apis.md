...
- **No extra dependency required**: Canvas is built into the browser and works in Felix mini apps without installing a game framework.
...
.effect-overlay {
  ...
  inset: 0;
  background: red;
  animation: effect-flash 0.3s ease;
  pointer-events: none;
}
```
...
- **Frees mouse buttons**: With movement handled by deltas alone, clicks can be mapped to game actions (launch, interact).
